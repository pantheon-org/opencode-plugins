 ...
 */
const groupBlocksByColor = (blocks) => {
 ...
 */
const mergeHorizontalRectangles = (blocks, blockSize) => {
    if (blocks.length === 0)
        return [];
    ...
    for (const block of sorted) {
        ...
        else if (block.y === current.y && // Same row
 ...
 */
const mergeVerticalRectangles = (rectangles) => {
    ...
    for (let i = 1; i < sorted.length; i++) {
        const rect = sorted[i];
        if (rect.x === current.x && // Same x position
 ...
 */
const rectanglesToCompoundPath = (rectangles) => {
 ...
 * ```
 */
export const optimizeBlocksToSVGPaths = (blocks, blockSize) => {
...
//# sourceMappingURL=svg-optimizer.js.map