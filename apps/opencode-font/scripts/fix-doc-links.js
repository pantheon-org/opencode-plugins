 ...
 */
function applyFixes(filePath, dryRun) {
...
function main() {
    ...
    for (const fix of fixes) {
        ...
        for (const file of files) {
            ...
            if (changes > 0) {
                console.log(`  ${dryRun ? 'Would fix' : 'Fixed'}: ${path.basename(file)} (${changes} change${changes > 1 ? 's' : ''})`);
                fixCount += changes;
                filesModified++;
            }
        }
        if (fixCount === 0) {
            ...
            console.log(`  ${colors.green}✓ ${fixCount} change${fixCount > 1 ? 's' : ''} ${dryRun ? 'would be' : ''} applied${colors.nc}`);
        }
        console.log('');
    ...
    if (dryRun) {
        ...
        console.log('');
        console.log(`Would modify ${filesModified} file${filesModified !== 1 ? 's' : ''} with ${totalChanges} total change${totalChanges !== 1 ? 's' : ''}`);
        console.log('');
        ...
        console.log('');
        console.log(`Modified ${filesModified} file${filesModified !== 1 ? 's' : ''} with ${totalChanges} total change${totalChanges !== 1 ? 's' : ''}`);
        console.log('');
...
main();
//# sourceMappingURL=fix-doc-links.js.map