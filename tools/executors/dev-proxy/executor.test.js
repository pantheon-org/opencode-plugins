...
function _fakeSpawn(cmd, args, opts) {
    return {
        kill: () => { },
        on: (ev, cb) => { },
    };
...
describe('dev-proxy executor with mocked runExecutor', () => {
    it('calls iterator.return() on shutdown', async () => {
        ...
        const mockSpawnSync = (cmd, args, opts) => ({ status: 0 });
    ...
    it('starts multiple projects and stops both on SIGINT', async () => {
        ...
        const mockRunExecutor = async (opts) => {
            if (opts && opts.project === 'pA')
                return iterA;
            if (opts && opts.project === 'pB')
                return iterB;
            return iterA;
        };
        const mockSpawnSync = (cmd, args, opts) => ({ status: 0 });
        const context = { root: process.cwd(), projectName: 'pA' };
        const beforeListeners = process.listeners('SIGINT').slice();
        const resPromise = runExecutor({ plugins: ['pA', 'pB'], __runExecutor: mockRunExecutor, __spawnSync: mockSpawnSync }, context);
        await new Promise((r) => setTimeout(r, 50));
        process.emit('SIGINT');
        await new Promise((r) => setTimeout(r, 50));
