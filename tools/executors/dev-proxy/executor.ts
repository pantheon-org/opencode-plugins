...
export default async function runExecutor(
  ...
  const resolved: { name: string; root?: string; config?: any }[] = [];
  for (const name of requestedPlugins) {
    if (name.includes(path.sep) || name.startsWith('.')) {
      resolved.push({ name });
      continue;
    }
    let info: { name: string; root?: string; config?: any } | null = null;
    if (devkit) {
      try {
        ...
        if (typeof devkit.getProjects === 'function') {
          ...
          if (projects) {
            if (typeof projects.get === 'function') {
              ...
              if (p) info = { name, root: p.root || p.sourceRoot, config: p };
        ...
        if (!info && typeof devkit.readProjectConfiguration === 'function') {
          try {
            ...
            if (cfg) info = { name, root: cfg.root || cfg.sourceRoot, config: cfg };
          } catch {}
        }
      } catch {}
    }
  ...
  for (const r of resolved) {
    if (r.root) console.log(`dev-proxy: resolved project ${r.name} -> ${r.root}`);
    else
    ...
    if (runExecutorImpl) {
      try {
        const maybe = runExecutorImpl(
          { project: projName, target: 'build', configuration: undefined, overrides: { watch: true } },
          context as any,
        );
        ...
        if (iterator && Symbol.asyncIterator in iterator) {
          (async () => {
            try {
              for await (const out of iterator as any) {
  ...
  if (res && (res as any).error) {
    console.error('Failed to run dev proxy runtime', (res as any).error);
    return { success: false };
  }
  return { success: res && (res as any).status === 0 };
}
