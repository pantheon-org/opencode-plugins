 ...
 */
const isAgentSpecConstructor = (value: unknown): boolean => {
 ...
 */
const validateAgentSpec = (spec: AgentSpec, filePath: string): void => {
  if (!spec.name || typeof spec.name !== 'string') {
    throw new Error(`Invalid agent name in ${filePath}: must be a non-empty string`);
  }

  if (!spec.config || typeof spec.config !== 'object') {
    throw new Error(`Invalid agent config in ${filePath}: must be an object`);
  }

  // Validate agent name format (kebab-case)
  if (!/^[a-z][a-z0-9-]*$/.test(spec.name)) {
    throw new Error(
      `Invalid agent name "${spec.name}" in ${filePath}: must be kebab-case (lowercase letters, numbers, and hyphens)`,
    );
  }
};
