 ...
 * @example
 * ```typescript
 * export const OpencodeJiraPlugin: Plugin = async (ctx) => {
...
export const OpencodeJiraPlugin: Plugin = async (ctx) => {
  ...
  console.log('[opencode-jira-plugin] Plugin loaded');
  console.log('[opencode-jira-plugin] Project:', project.id);
  console.log('[opencode-jira-plugin] Worktree:', worktree);

  // Return plugin hooks
  return {
    // Example: Handle session events
    event: async ({ event }) => {
      console.log('[opencode-jira-plugin] Event:', event.type);
    },
