...
export const OpencodeCorePlugin: Plugin = async (ctx) => {
  ...
  console.log('[opencode-core-plugin] Plugin loaded');
  console.log('[opencode-core-plugin] Project:', project.id);
  console.log('[opencode-core-plugin] Worktree:', worktree);

  // Return plugin hooks
  return {
    // Example: Handle session events
    event: async ({ event }) => {
      console.log('[opencode-core-plugin] Event:', event.type);
    },
