...
const copyIfMissing = async (sourcePath: string, targetDir: string, filename: string): Promise<boolean> => {
 ...
 * ```
 */
export const ensureSoundAvailable = async (filename: string, dataDir?: string): Promise<boolean> => {
...
export const installBundledSoundsIfMissing = async (dataDir?: string): Promise<number> => {
  ...
  const installPromises = entries.map((entry) => processEntry(entry, bundledDataDir, effectiveDataDir));
