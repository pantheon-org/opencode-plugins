...
export const setGlobalFetch = (fetchImpl: FetchImpl) => {
  const orig = typeof globalThis !== 'undefined' ? (globalThis as unknown as { fetch?: FetchImpl }).fetch : undefined;
  if (typeof globalThis !== 'undefined') (globalThis as unknown as { fetch?: FetchImpl }).fetch = fetchImpl;
  return () => {
    if (typeof globalThis !== 'undefined') (globalThis as unknown as { fetch?: FetchImpl }).fetch = orig;
  };
};
 ...
 */
export const withPlatform = async <T = unknown>(platform: string, fn: () => Promise<T> | T): Promise<T> => {
 ...
 */
export const writeTempFileForFaction = (baseDir: string, faction: string, filename: string, contents = 'x') => {
