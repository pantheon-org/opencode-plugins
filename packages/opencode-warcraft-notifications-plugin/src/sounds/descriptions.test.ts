...
describe('Sound Descriptions', () => {
  ...
  describe('description quality', () => {
    ...
    test('should include punctuation in descriptions', () => {
      const descriptionsWithPunctuation = Object.values(soundDescriptions).filter((desc) => desc.match(/[.!?]$/));

      // Most descriptions should end with punctuation
      expect(descriptionsWithPunctuation.length).toBeGreaterThan(Object.values(soundDescriptions).length * 0.8);
    });
